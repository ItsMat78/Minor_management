...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            // Detect Keys
            // "K" seemed to be the S.No key in inspection
            ...
            if (currentGroupId && keyName && row[keyName] && keyRoll && row[keyRoll]) {
                ...
                if (!groups.has(currentGroupId)) {
                    groups.set(currentGroupId, {
                        ...
                        projectTitle: currentProjectTitle || 'Minor Project',
        ...
        for (const [gId, gData] of groups) {
            ...
            if (!pTitle || pTitle === 'TBD' || pTitle === 'null') pTitle = `Minor Project Group ${gId}`;
